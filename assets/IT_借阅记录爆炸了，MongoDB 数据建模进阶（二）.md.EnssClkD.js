import{_ as s,o as a,c as n,a7 as p}from"./chunks/framework.DHDObbvy.js";const i="/images/2026/model3.png",e="/images/2026/model3_bucket.png",t="/images/2026/model3_archive.png",v=JSON.parse('{"title":"借阅记录爆炸了，MongoDB 怎么办？MongoDB 数据建模进阶（二）","description":"","frontmatter":{"title":"借阅记录爆炸了，MongoDB 怎么办？MongoDB 数据建模进阶（二）","date":"2026-09-18T14:06:16.000Z","tags":["MongoDB"],"categories":["IT"]},"headers":[],"relativePath":"IT/借阅记录爆炸了，MongoDB 数据建模进阶（二）.md","filePath":"IT/借阅记录爆炸了，MongoDB 数据建模进阶（二）.md","lastUpdated":1790150473000}'),l={name:"IT/借阅记录爆炸了，MongoDB 数据建模进阶（二）.md"},h=p('<p><img src="'+i+`" alt=""></p><h1 id="借阅记录爆炸了-mongodb-怎么办" tabindex="-1">借阅记录爆炸了，MongoDB 怎么办？ <a class="header-anchor" href="#借阅记录爆炸了-mongodb-怎么办" aria-label="Permalink to &quot;借阅记录爆炸了，MongoDB 怎么办？&quot;">​</a></h1><h2 id="mongodb-数据建模进阶-二" tabindex="-1">MongoDB 数据建模进阶（二） <a class="header-anchor" href="#mongodb-数据建模进阶-二" aria-label="Permalink to &quot;MongoDB 数据建模进阶（二）&quot;">​</a></h2><p>上一篇，我们解决了两个问题：</p><p><strong>一本书的数据太多怎么办？</strong></p><p>可以用 <strong>Subset Pattern</strong>，只把常用的数据放进主文档。</p><p><strong>少数数据特别大怎么办？</strong></p><p>可以用 <strong>Outlier Pattern</strong>，把特殊数据单独处理。</p><p>但如果问题再扩大一点呢？</p><p>图书馆每天都有几千、几万条借阅记录。</p><p>一个月几十万条，一年几百万条。</p><p>这时候麻烦的已经不是“一本书太大”，而是：</p><blockquote><p><strong>整个借阅记录库都在不断膨胀。</strong></p></blockquote><p>那这些数据应该怎么组织？</p><hr><h2 id="一、借阅记录越来越多-能不能-分箱-保存" tabindex="-1">一、借阅记录越来越多，能不能“分箱”保存？ <a class="header-anchor" href="#一、借阅记录越来越多-能不能-分箱-保存" aria-label="Permalink to &quot;一、借阅记录越来越多，能不能“分箱”保存？&quot;">​</a></h2><p>想象一下，图书馆上线了一套新的借阅系统。</p><p>刚开始，每天只有几百条记录：</p><div class="language-text vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">text</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>9月15日：386 条</span></span>
...
<span class="line"><span>2025 年</span></span></code></pre></div><p>至于 2016 年的借阅记录，可能几个月都不会有人碰一次。</p><p>那有没有必要让这些十年前的数据，和今天的数据一样“活跃”？</p><p>其实没必要。</p><p><img src="`+t+`" alt=""></p><h3 id="图书馆里的做法更直观" tabindex="-1">图书馆里的做法更直观 <a class="header-anchor" href="#图书馆里的做法更直观" aria-label="Permalink to &quot;图书馆里的做法更直观&quot;">​</a></h3><p>现实中的图书馆不会把几十年前的所有资料都堆在服务台旁边。</p><p>通常会分成：</p><div class="language-text vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">text</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>日常使用区</span></span>
<span class="line"><span>↓</span></span>
...
<span class="line"><span>        ↓</span></span>
<span class="line"><span>进入归档区</span></span></code></pre></div><p>也就是说：</p><blockquote><p><strong>Bucket 负责“怎么装”，Archive 负责“老了以后放哪”。</strong></p></blockquote><hr><h2 id="四、是不是数据一多-就必须上-pattern" tabindex="-1">四、是不是数据一多，就必须上 Pattern？ <a class="header-anchor" href="#四、是不是数据一多-就必须上-pattern" aria-label="Permalink to &quot;四、是不是数据一多，就必须上 Pattern？&quot;">​</a></h2><p>当然不是。</p><p>假设一家小图书馆每天只有：</p><div class="language-text vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">text</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>50 条借阅记录</span></span></code></pre></div><p>一年也就一两万条。</p><p>这种情况下，完全没必要为了使用 Bucket Pattern，而把数据模型搞得很复杂。</p><p>同样，如果五年前的借阅记录现在每天都有人查询，也没必要为了“Archive Pattern”强行搬走。</p><p>所以真正应该问的不是：</p><blockquote><p>“这个项目用了几个 Pattern？”</p></blockquote><p>而是：</p><blockquote><p><strong>现在的数据规模、查询方式和生命周期，真的需要吗？</strong></p></blockquote><p>MongoDB 数据建模一直强调一个原则：</p><blockquote><p><strong>先看业务怎么访问数据，再决定数据怎么组织。</strong></p></blockquote><p>Pattern 只是解决问题的工具，不是数据库设计的“必做题”。</p><hr><h1 id="五、最后记住两个名字" tabindex="-1">五、最后记住两个名字 <a class="header-anchor" href="#五、最后记住两个名字" aria-label="Permalink to &quot;五、最后记住两个名字&quot;">​</a></h1><p>这一篇其实只需要记住两句话。</p><p><strong>Bucket Pattern（桶模式）</strong></p><blockquote><p><strong>数据越来越多，就按照合适的规则分成一桶一桶。</strong></p></blockquote><p>解决：</p><blockquote><p><strong>正在增长的数据怎么组织。</strong></p></blockquote><hr><p><strong>Archive Pattern（归档模式）</strong></p><blockquote><p><strong>数据已经很久不活跃，就把它单独管理。</strong></p></blockquote><p>解决：</p><blockquote><p><strong>历史数据怎么管理。</strong></p></blockquote><p>放回我们的图书馆：</p><div class="language-text vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">text</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>借阅记录不断增加</span></span>
<span class="line"><span>        ↓</span></span>
